...
function Item({ groupedItem }: { groupedItem: [productKey: string, items: CartItem[]] }) {
	...
	if (matchedItem) {
		return (
			<div className="flex w-full justify-between rounded-md border-2 border-gray-100 bg-gray-50 text-gray-800">
					...
					<div className="flex flex-col items-end justify-between">
						...
						<div className="flex gap-2">
							...
							<button
								...
								onClick={() =>
									setCart((prevItems) => {
										const matchedItems = prevItems
											.filter(({ productKey }) => productKey === matchedItem.productKey)
											.slice(0, -1);

										const nonMatchingItems = prevItems.filter(
											({ productKey }) => productKey !== matchedItem.productKey
										);

										return [...matchedItems, ...nonMatchingItems];
									})
								}>
								Remove
							</button>
